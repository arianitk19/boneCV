...
(function (global) {
    ...
    function getSidebarHTML(activePage) {
        ...
        <div class="flex flex-col h-full">
            <div class="sb-brand-row px-6 pt-6 pb-4 flex items-center gap-3">
                ${brandMark()}
                <div>
            ...
            <div class="px-4 py-4 border-t border-white/[0.06]">
                ...
                <div class="text-[11px] text-white/30">© ${new Date().getFullYear()} boneCV Pro • v1.9</div>
            </div>
        </div>`;
    }
    ...
    function confirmDialog(opts) {
        return new Promise(resolve => {
            ...
            const onKey = (e) => {
                ...
                if (e.key === 'Tab') {
                    // Keep focus inside the dialog (2 buttons).
            ...
            // Focus the safe action first — especially for destructive dialogs.
            requestAnimationFrame(() => wrap.querySelector('[data-act="cancel"]').focus());
        });
    }
    ...
    let updateAccepted = false; // guards against reload on first SW install
    ...
    function initPWA(opts) {
        ...
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                ...
                // Reload once when the new SW takes control — but only after
                // the user explicitly accepted the update. Without this guard
                // clients.claim() on first install would reload the page.
                let reloaded = false;
    ...
    function injectGlobalCSS() {
        ...
        @media print{ #bonecv-sheet, #bonecv-help, #bonecv-help-btn{ display:none !important; } }
        /* ---- CREATE SHEET ---- */
        ...
        /* ---- Press feedback (pure CSS — never blocks taps/navigation) ---- */
        .btn, .bonecv-tap, .qa, .sh-opt, .nav-link, .hp-chip { -webkit-tap-highlight-color:transparent; transition:transform .12s ease, background .2s; }
        .btn:active, .bonecv-tap:active, .qa:active, .sh-opt:active, .nav-link:active, .hp-chip:active{ transform:scale(.97); }
        /* ---- Help button ---- */
        ...
        .hp-x{ border:none; background:rgba(255,255,255,0.06); color:inherit; width:44px; height:44px; border-radius:13px; cursor:pointer; font-size:16px; }
        ...
        /* ---- Sidebar extras ---- */
        .sb-lang{ width:48px; height:40px; border-radius:10px; border:1px solid rgba(255,255,255,0.1); background:rgba(255,255,255,0.04);
        ...
        #menuBtn .bm-lines{ position:relative; width:20px; height:14px; display:block; }
        #menuBtn .bm-lines span{ position:absolute; left:0; width:100%; height:2px; border-radius:2px; background:var(--accent);
            transition:transform .38s cubic-bezier(.16,1,.3,1), opacity .2s, top .38s cubic-bezier(.16,1,.3,1); }
        #menuBtn .bm-lines span:nth-child(1){ top:0; }
        #menuBtn .bm-lines span:nth-child(2){ top:6px; }
        #menuBtn .bm-lines span:nth-child(3){ top:12px; }
        #menuBtn.is-x .bm-lines span:nth-child(1){ top:6px; transform:rotate(45deg); }
        #menuBtn.is-x .bm-lines span:nth-child(2){ opacity:0; transform:scaleX(0); }
        #menuBtn.is-x .bm-lines span:nth-child(3){ top:6px; transform:rotate(-45deg); }
        body.bonecv-menu-open{ overflow:hidden; }
        @media (max-width:1023px){ #sidebar .sb-brand-row{ padding-top:calc(env(safe-area-inset-top) + 4.25rem) !important; } }
    ...
    /* ============================================================
     * CREATE SHEET — opened from the sidebar "Create document" CTA
     * ============================================================ */
    function openCreateSheet() {
        ...
        sheet.setAttribute('aria-label', t('sheet.title'));
        ...
        requestAnimationFrame(() => {
            ...
            const first = sheet.querySelector('.sh-opt');
            if (first) first.focus();
        });
    }

    /* ============================================================
     * MOBILE MENU — animated hamburger↔X, toggle, smooth
     * ============================================================ */
    function initMobileMenu() {
        const btn = document.getElementById('menuBtn');
        const sb = document.getElementById('sidebar');
        const ov = document.getElementById('sbOverlay');
        if (!btn || !sb || btn.dataset.bmReady) return;
        btn.dataset.bmReady = '1';
        btn.innerHTML = '<span class="bm-lines"><span></span><span></span><span></span></span>';
        ...
        const setOpen = (open) => {
            ...
            document.body.classList.toggle('bonecv-menu-open', open);
            btn.setAttribute('aria-expanded', open ? 'true' : 'false');
        };
        btn.onclick = (e) => { e.preventDefault(); setOpen(!sb.classList.contains('open')); };
        if (ov) ov.onclick = () => setOpen(false);
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && sb.classList.contains('open')) {
                setOpen(false);
                btn.focus(); // return focus so keyboard users don't get lost
            }
        });
    ...
    function openHelp() {
        ...
        const onHelpKey = (e) => { if (e.key === 'Escape') { e.stopPropagation(); close(); } };
    ...
    global.bonecv = {
        ...
        formatDateSq, formatDateTimeSq, toast, confirmDialog,
        getSidebarHTML, mountSidebar,
