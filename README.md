...
> A private, offline‑first PWA for creating professional documents in Albanian. Bilingual (SQ/EN), dark/light themes, installable.

## Struktura e projektit / Project structure

Struktura është e sheshtë (flat) — të gjitha faqet, skriptet dhe ikonat rrinë në një dosje. Kjo është qëllimisht: service worker-i duhet të jetë në rrënjë që të kontrollojë të gjitha faqet, dhe lidhjet mes faqeve mbeten të thjeshta.
...
├── manifest.json         # Web App Manifest (instalim / PWA)
├── service-worker.js     # Service worker (cache offline-first)
├── master.js             # Logjika e përbashkët (state, i18n, temë, UI)
├── icon.svg              # Ikona e aplikacionit (vektoriale, çdo madhësi)
├── icon-mono.svg         # Varianti monokrom (njoftime)
└── README.md
```

Të gjitha faqet ndajnë `master.js`, i cili menaxhon: ruajtjen lokale (localStorage), përkthimin Shqip/English në kohë reale, motorin e temës, sidebar-in, qendrën e ndihmës me manual, dhe regjistrimin e service worker-it. Ikona `icon.svg` përdoret si favicon, apple-touch-icon dhe ikonë e manifest-it (`any` + `maskable`).
...
- **E instalueshme** — banner "Instalo" + ikonë vektoriale maskable.
...
Ikonat janë **SVG vektoriale** (`icon.svg`, `icon-mono.svg`) — të pastra në çdo madhësi dhe pa nevojë për skedarë PNG. Nëse dëshiron ta paketosh për **Google Play (TWA)**, gjenero PNG 512×512 dhe 192×192 nga `icon.svg` gjatë ndërtimit (p.sh. me Bubblewrap / një konvertues SVG→PNG).
